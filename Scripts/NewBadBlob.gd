...
func get_hit(damage=1, sprayAngle=0, offset=Vector2.ZERO, tricks=[], multikill=1):
	...
	if damage < 2:
		sp.emitting = false
		sp.get_node("OtherSpray").emitting = true
		...
		if tricks.size() > 0:
			for trick in tricks:
				...
				if trick == "Air":
					...
					trickscore += 3 * baseScore
				elif trick == "Long shot":
					trickscore += 2 * baseScore
		
		# multikill here
		
		trickscore *= get_parent().multiplier
		
		t.get_node("Label").text = trickname + "\n+" + str(trickscore)
		get_parent().add_kill(trickscore)
		
		Globals.totalKilled += 1
		queue_free()
...
func _on_BadBlob_body_entered(body):
